export class GlobalConstants {

    public static serverAddress: string = "https://penielbeach.com.br";
    

    public static siteApiURL: string = GlobalConstants.serverAddress + "/wp-json";

    // OK public static canalYoutubeURL: string = "https://www.googleapis.com/youtube/v3/search?channelId=UCexdKUQ25mhIv42KS6nVgLQ&order=date&part=snippet&type=video&maxResults=10&key=AIzaSyAihjQWgnE9RURPK_atUU7QPkwE44qK7nI";
 
    public static canalYoutubeURL: string ="https://www.youtube.com/feeds/videos.xml?channel_id=UCexdKUQ25mhIv42KS6nVgLQ";

    


    // AIzaSyAihjQWgnE9RURPK_atUU7QPkwE44qK7nI
}






